...
def template_manager():
    """Create a template manager mock with proper configuration."""
    manager = MagicMock(spec=TemplateManager)
    manager.render_template.return_value = "<html>Mocked template content</html>"
    return manager

@pytest.fixture
def mock_smtp():
    """Create a properly configured SMTP mock that supports context manager."""
    smtp_mock = MagicMock()
    # Configure the mock to handle context manager protocol
    smtp_mock.__enter__.return_value = smtp_mock
    smtp_mock.__exit__.return_value = None
    return smtp_mock

@pytest.fixture
def email_service(template_manager):
    """Create email service with properly mocked SMTP."""
    with patch('smtplib.SMTP') as mock_smtp_class:
        # Configure the SMTP mock
        mock_server = MagicMock()
        mock_server.__enter__.return_value = mock_server
        mock_server.__exit__.return_value = None
        
        # Make the SMTP class return our configured mock
...
def normal_user_token_headers(test_user):
    token = create_access_token(
        data={"sub": str(test_user.id), "role": UserRole.AUTHENTICATED.value}
    )
